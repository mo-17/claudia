...
export const CheckpointSettings: React.FC<CheckpointSettingsProps> = ({
  ...
  const loadSettings = async () => {
    try {
      ...
      setError(t('checkpointSettings.errors.loadFailed'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      setSuccessMessage(t('checkpointSettings.success.saved'));
      setTimeout(() => setSuccessMessage(null), 3000);
    } catch (err) {
      console.error("Failed to save checkpoint settings:", err);
      setError(t('checkpointSettings.errors.saveFailed'));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleCleanup = async () => {
    try {
      ...
      setSuccessMessage(t('checkpointSettings.success.cleaned', { count: removed }));
      setTimeout(() => setSuccessMessage(null), 3000);
      ...
      setError(t('checkpointSettings.errors.cleanupFailed'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="rounded-lg border border-yellow-500/50 bg-yellow-500/10 p-3">
        <div className="flex items-start gap-2">
          ...
          <div className="text-xs">
            <p className="font-medium text-yellow-600">{t('checkpointSettings.experimental.title')}</p>
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            <Label htmlFor="auto-checkpoint">{t('checkpointSettings.autoCheckpoint.title')}</Label>
        ...
        <div className="space-y-2">
          <Label htmlFor="strategy">{t('checkpointSettings.strategy.title')}</Label>
          <SelectComponent
      ...
      <div className="border-t pt-6 space-y-4">
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            <Label>{t('checkpointSettings.storage.title')}</Label>
            <p className="text-sm text-muted-foreground">
              {t('checkpointSettings.storage.total', { count: totalCheckpoints })}
            </p>
          </div>
        ...
        {/* Cleanup settings */}
        <div className="space-y-2">
          <Label htmlFor="keep-count">{t('checkpointSettings.cleanup.title')}</Label>
          <div className="flex gap-2">
            ...
            >
              ...
              {t('checkpointSettings.cleanup.button')}
            </Button>
          </div>
          <p className="text-xs text-muted-foreground">
            {t('checkpointSettings.cleanup.description', { count: keepCount })}
          </p>
        </div>
      </div>
    </motion.div>
