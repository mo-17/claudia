          ...
          >
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              <TabsList className="grid w-full grid-cols-5">
                ...
                <TabsTrigger value="sessions">{t('usage.bySession')}</TabsTrigger>
                        ...
                        <div key={model.model} className="flex items-center justify-between">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-xs text-muted-foreground">
                              {t('usage.sessionCount', { count: model.session_count })}
                            </span>
                          </div>
                    ...
                    {stats.by_model.map((model) => (
                      <div key={model.model} className="space-y-2">
                        ...
                        <div className="grid grid-cols-4 gap-2 text-xs">
                          <div>
                            <span className="text-muted-foreground">{t('usage.inputShort')}: </span>
                            <span className="font-medium">{formatTokens(model.input_tokens)}</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">{t('usage.outputShort')}: </span>
                    ...
                    {stats.by_project.map((project) => (
                      <div key={project.project_path} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {t('usage.perSession', { cost: formatCurrency(project.total_cost / project.session_count) })}
                          </p>
                        </div>
                      </div>
                    ))}
