...
export const AgentRunView: React.FC<AgentRunViewProps> = ({
  ...
  const loadRun = async () => {
    try {
      ...
      setError(t('agentRunView.failedToLoad'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCopyAsMarkdown = async () => {
    ...
    markdown += `**${t('agentRunView.task')}:** ${run.task}\n`;
    markdown += `**${t('agentRunView.model')}:** ${run.model}\n`;
    markdown += `**${t('agentRunView.status')}:** ${run.status}\n`;
    if (run.metrics) {
      markdown += `**${t('agentRunView.tokens')}:** ${run.metrics.total_tokens || t('agentRunView.na')}\n`;
      markdown += `**${t('agentRunView.cost')}:** $${run.metrics.cost_usd?.toFixed(4) || t('agentRunView.na')}\n`;
    }
    markdown += `**${t('agentRunView.date')}:** ${new Date(run.created_at).toISOString()}\n\n`;
    markdown += `---\n\n`;

    for (const msg of messages) {
      if (msg.type === "system" && msg.subtype === "init") {
        markdown += `## ${t('agentRunView.systemInitialization')}\n\n`;
        markdown += `- ${t('agentRunView.sessionId')}: \`${msg.session_id || t('agentRunView.na')}\`\n`;
        markdown += `- ${t('agentRunView.model')}: \`${msg.model || 'default'}\`\n`;
        ...
        if (msg.message.usage) {
          markdown += `*${t('agentRunView.tokensUsage', { input: msg.message.usage.input_tokens, output: msg.message.usage.output_tokens })}*\n\n`;
        }
    ...
    try {
      ...
      if (success) {
        ...
        const stopMessage: ClaudeStreamMessage = {
          ...
          subtype: "error",
          is_error: true,
          result: t('agentRunView.stoppedByUser'),
          duration_ms: 0,
          usage: {
            input_tokens: 0,
            output_tokens: 0
        ...
        <Card className="m-4">
          <CardContent className="p-4">
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <h3 className="text-sm font-medium">{t('agentRunView.task')}:</h3>
                <p className="text-sm text-muted-foreground flex-1">{run.task}</p>
                <Badge variant="outline" className="text-xs">
                  {run.model === 'opus' ? t('agentRunView.claude4Opus') : t('agentRunView.claude4Sonnet')}
                </Badge>
              </div>
